...
def calculate_bandgap(atoms: Atoms, method: str = 'pbe', kpts_density: float = 0.0) -> Dict[str, float]:
    cell = atoms_to_pyscf_cell(atoms)    
    
    # Use gamma point only for quick calculations, or add k-points for better accuracy
    if kpts_density > 0:
        # Generate k-points based on cell size for better sampling
        cell_volume = np.linalg.det(cell.a)
        nkpts = max(1, int(kpts_density * cell_volume))
        kpts = cell.make_kpts([nkpts, nkpts, nkpts])
        logger.info(f"Using {len(kpts)} k-points for better accuracy")
    else:
        kpts = np.array([[0, 0, 0]])  # Gamma point only
        logger.info("Using gamma point only (fast but less accurate)")
    ...
    if method.lower() == 'pbe':
        ...
        mf.xc = 'b3lyp'
        mf.exxdiv = 'ewald'
    elif method.lower() == 'hse06':
        mf.xc = 'hse06'  # HSE06 is often better for band gaps
        mf.exxdiv = 'ewald'
    elif method.lower() == 'pbe0_25':
        # PBE0 with 25% exact exchange (often better than standard PBE0)
        mf.xc = 'pbe0'
        mf.exxdiv = 'ewald'
        # Note: PBE0 already has 25% exact exchange
    else:
        mf.xc = method
        
    mf = mf.density_fit()
    # Tighter convergence for better accuracy
    mf.conv_tol = 1e-6  # Tighter convergence

    mf.max_cycle = 200  # More iterations if needed
    mf.kernel()
...
def process_structures(xyz_file: str, output_file: str = None, method: str = 'pbe', kpts_density: float = 0.0) -> pd.DataFrame:
    ...
    for i, atoms in enumerate(structures):
        ...
        try:
            result = calculate_bandgap(atoms, method, kpts_density)
...
def main():
    ...
    parser.add_argument('--method', default='pbe', choices=['pbe', 'pbe0', 'b3lyp', 'hse06', 'pbe0_25'])
    parser.add_argument('--kpts_density', type=float, default=0.0, 
                       help='K-point density for better accuracy (0.0 = gamma point only)')
    ...
    if args.test:
        ...
        df = process_structures(test_file, args.output, args.method, args.kpts_density)
        os.remove(test_file)
    else:
        df = process_structures(args.xyz_file, args.output, args.method, args.kpts_density)
