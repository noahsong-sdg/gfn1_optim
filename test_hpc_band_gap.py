#!/usr/bin/env python3
"""
Test script for HPC-optimized band gap calculator
...
from hpc_band_gap_calculator import HPCBandGapCalculator
import ase.io
import numpy as np

def test_single_structure():
    """Test band gap calculation with a single structure"""
    print("Testing HPC-optimized band gap calculator with single CdS structure...")
    ...
    # Initialize calculator with HPC settings
    calculator = HPCBandGapCalculator(
        ...
        max_memory=8000,  # 8 GB
        conv_tol=1e-6,
        max_cycle=50  # Reduced for testing
    )
...
def test_multiple_structures(n_structures=3):
    ...
    if successful:
        ...
        print("\nIndividual results:")
        for r in successful:
            print(f"  Structure {r['structure_id']}: {r['band_gap']:.3f} eV")
...
if __name__ == "__main__":
    ...
    if result and result['error'] is None:
        test_multiple_structures(3)  # Reduced for testing
    else:
